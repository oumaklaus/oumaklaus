<div align="center">

<img src="https://count.getloli.com/@oumaklaus?name=oumaklaus&theme=rule34&padding=1&offset=0&align=center&scale=1&pixelated=1&darkmode=auto" alt="Profile view counter">

# Clause

**CS Student @ Kenyatta University** &nbsp;·&nbsp; *Machine Learning*

Undergrad in Nairobi — mostly Python, mostly models, and the systems underneath them.

[GitHub](https://github.com/oumaklaus) &nbsp;·&nbsp; [LinkedIn](https://www.linkedin.com/in/oumaclause) &nbsp;·&nbsp; [Discord](https://discord.com/users/ufwzuko)

<br>

### Tech Stack

<img src="https://skillicons.dev/icons?i=python,cpp,java,js,pytorch,tensorflow,sklearn,mysql,linux,git&perline=5" alt="Python, C++, Java, JavaScript, PyTorch, TensorFlow, scikit-learn, MySQL, Linux, Git">

</div>

---

### Projects

**[reconx](https://github.com/oumaklaus/reconx)** — Python
<!-- TODO: one line on what reconx actually does -->

**[ml-internship](https://github.com/oumaklaus/ml-internship)** — Jupyter Notebook
<!-- TODO: one line — what you built, what dataset, what you took away -->

---

<div align="center">

<img src="https://media.giphy.com/media/v1.Y2lkPTc5MGI3NjExZWx4eGZ5c3V4d3N0d2M4N3Z3aG5vN2d3eXh4dXJ4Z2F4Y2Nq/13HgwGsXF0aiGY/giphy.gif" width="480" alt="Anime">

</div>
